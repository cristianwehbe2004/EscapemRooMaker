...
public class PlayerSessionServiceTests
{
    ...
    [Fact]
    public async Task QuickStartAsync_ShouldUseSevenMinuteMetadataForHardRoom()
    {
        var context = await CreateContextWithPublishedRoomAsync("""
            {
              "room": {
        ...
        var summary = await service.QuickStartAsync(
            new CreateSessionRequest { DisplayName = "Host" },
            new PlayerIdentity { ActorId = "host-5", DisplayName = "Host", IsAuthenticated = false });

        summary.DurationMinutes.Should().Be(7);
        summary.RemainingSeconds.Should().Be(420);
    }
